	     ...
	     OfxPropertySetHandle drawArgs)
{
  ...
  gPropHost->propGetPointer(drawArgs, kOfxInteractPropDrawContext, 0, (void**)&context);
  
  // if the we have selected the Xhair, draw it highlit
  OfxRGBAColourF col;
  if(data->selected)
  {
      if(gDrawSuite->getColour(context, kOfxStandardColourOverlayActive, &col) != kOfxStatOK)
	  col = {1, 1, 1, 1};
      gDrawSuite->setColour(context, &col);
      gDrawSuite->setLineStipple(context, kOfxDrawLineStipplePatternDot);
      OfxPointD points[2]
      {
	  {x-dx*10, y-dy*5},
	  {x+dx*10, y+dy*5},
      };
      gDrawSuite->draw(context, kOfxDrawPrimitiveEllipse, points, 2);
      gDrawSuite->setLineStipple(context, kOfxDrawLineStipplePatternSolid);
  }
  else
  {
      if(gDrawSuite->getColour(context, kOfxStandardColourOverlayText, &col) != kOfxStatOK)
	  col = {1, 1, 1, 1};
      gDrawSuite->setColour(context, &col);
      OfxPointD pos {x+dx/4, y+dy/4};
      gDrawSuite->drawText(context, "Click me!", &pos, kOfxDrawTextAlignmentLeft | kOfxDrawTextAlignmentBaseline);

      // Now get line colour
      if(gDrawSuite->getColour(context, kOfxStandardColourOverlayDeselected, &col) != kOfxStatOK)
	  col = {1, 0, 0, 1}; 
      gDrawSuite->setColour(context, &col);
  }
  ...
  gDrawSuite->draw(context, kOfxDrawPrimitiveLines, points, 4);
...
static OfxStatus
drawSuiteMain(const char *action,  const void *handle, OfxPropertySetHandle inArgs, OfxPropertySetHandle /*outArgs*/)
{
...
describe(OfxImageEffectHandle effect)
{
  ...
  gDrawSuite =
    (OfxDrawSuiteV1*)gHost->fetchSuite(gHost->host, kOfxDrawSuite, 1);
  ...
  gPropHost->propSetString(effectProps, kOfxPropLabel, 0, "OFX DrawSuite Example");
  ...
  gPropHost->propSetPointer(effectProps, kOfxImageEffectPluginPropOverlayInteractV2, 0,  (void *) drawSuiteMain);
...
{       
  ...
  1,
  "uk.co.thefoundry.BasicDrawSuitePlugin",
  1,
  0,
  setHostFunc,
  pluginMain
