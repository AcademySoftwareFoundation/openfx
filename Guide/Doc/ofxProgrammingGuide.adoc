...
This book assumes you can program in the `C` language and are
...
This book describes the basic OFX plug-in architecture and the
...
== Basic Plugin Machinery
The basic plugin machinery establishes communications between a plugin and a host application. It very flexible and
many different kinds of plugin APIs can be layered on it. It relatively easily allows for backwards compatibility by strong versioning and genericity. 

An example plugin will be used to illustrate how all this machinery works, and its source can be found in the source code 
file `GuideExamples/Machinery/machinery.cpp`.  FIXME LINKY
...
{       
  ...
  1,                                        
  "org.openeffects:MachineryExamplePlugin", 
  1,                                        
  0,                                        
  SetHostFunc,                              
  MainEntryPoint                            
...
plugin to be used simultaneously within a host application. FIXME LINK TO VERSIONING GUFF
...
It is then setting the string propery `**kOfxPropLabel**` to be "OFX Machinery Example". 
...
continual string lookup), but the flexibility it gives is worth it.
...
Infact only four actions need to be trapped for an image effect plugin footnote:[kOfxLoadAction, kOfxActionDescribe, kOfxImageEffectActionDescribeInContext and one of kOfxImageEffectActionIsIdentity or kOfxImageEffectActionRender], but our machinery plugin is trapping moew for illustrative purposes.

What is on the property sets, and what the handle should be down cast to depends on the action being called. Some actions have no arguments (eg: the kOfxLoadAcrtion), while others have in and out arguments, e.g. the kOfxImageEffectActionIsIdentity. 
...
of systems such as editors which can drop effects directly onto a clip in a timeline. For more complex systems, e.g. a node graph compositor, 
...
=== Packaging A Plugin
Plugins are packaged up inside a dynamic library that sits inside a directory structure which conforms to Apple's ".bundle" format. 
The directory structure allows you to ship plugins that target multiple OS's as one, as well as giving a location to store any resource
that the plugin may want to ship alongside the binary.

==== Dynamic Library Format
Plugins are contained within a dynamic library, the exact form depends on the operating system being used. 

  * Microsoft Windows, as ".dll" dynamically linked libraries, 
  * Apple OSX, as Mach-O binary bundles, footnote:[a binary bundle is distinct to a bundle directory structure, when linking a binary bundle on OSX, use the -bundle compiler switch]
  * LINUX, as ELF dynamic shared objects. 

The filename of all the dynamic libraries must end with the suffix ".ofx".

==== Bundle Directory Structure
Each plug-in binary is contained in an Apple Mac OS X bundle 
directory hierarchy, which looks something like.....
...
Where...
  ...
  * PLUGIN.png - is a Portable Network Graphics image to use as an icon for the plug-in in the binary which has a matching pluginIdentifier field in the OfxPlugin struct,
  * PLUGIN.svg - is a Scalable Vector Graphics image to use as an icon for the plug-in in the binary which has a matching pluginIdentifier field in the OfxPlugin struct
  * ARCHITECTURE is the specific operating system architecture the plug-in was built for, these are currently...
    ** MacOS - for Apple Macintosh OS X (compiled 32 or 64 bit)
